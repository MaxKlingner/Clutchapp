...
function mapTutorProfile(row) {
  ...
  const teachingFormats = Array.isArray(row.teaching_formats)
    ? row.teaching_formats.filter(Boolean).map(String)
    : [];
  ...
  return {
    ...
    postalCode:
      row.postal_code != null && String(row.postal_code).trim()
...
export async function fetchTutorProfiles({ excludeClerkId } = {}) {
  ...
  const { data, error } = await client
    .from('profiles')
    .select(
      'id, clerk_id, full_name, subject, hourly_rate, rating, review_count, role, bio, specialties, study_year, avatar_url, city, postal_code, latitude, longitude, teaching_formats, created_at'
    )
    .eq('role', 'tutor')
...
export async function updateTutorProfile(clerkId, updates) {
  ...
  const teachingFormats = Array.isArray(updates.teachingFormats)
    ? updates.teachingFormats.filter(Boolean)
    : undefined;
  ...
  if (updates.postalCode !== undefined) {
    payload.postal_code = updates.postalCode?.trim() || null;
  }
  ...
  if (updates.longitude !== undefined) {
    ...
    payload.longitude = Number.isFinite(lng) ? lng : null;
  }
  if (teachingFormats !== undefined) {
...
/**
 * Liste les IDs de tuteurs favoris pour un parent (Clerk user_id).
 * @param {string} userId Clerk id
 * @returns {Promise<string[]>}
 */
export async function fetchFavoriteTutorIds(userId) {
  const client = requireSupabase();
  if (!userId) return [];
  ...
  if (error) {
    throw new Error(error.message || 'Impossible de charger les favoris.');
  }

  return (data ?? []).map((row) => String(row.tutor_id));
}

/**
 * Liste les tuteurs favoris (profils joints).
 * @param {string} userId Clerk id
 */
export async function fetchFavoriteTutors(userId) {
  const client = requireSupabase();
  if (!userId) return [];
  ...
  const { data, error } = await client
    ...
    .select(
      ...
      created_at,
      tutor:profiles (
        id, clerk_id, full_name, subject, hourly_rate, rating, review_count,
        role, bio, specialties, study_year, avatar_url,
        city, postal_code, latitude, longitude, teaching_formats, created_at
      )
    `
    )
    .eq('user_id', userId)
  ...
  return (data ?? [])
    .map((row) => ({
      ...
      tutorId: String(row.tutor_id),
      createdAt: row.created_at,
      tutor: mapTutorProfile(row.tutor),
    }))
    .filter((row) => row.tutor);
}

/**
 * Ajoute un tuteur aux favoris (idempotent grâce à UNIQUE).
 */
export async function addFavoriteTutor(userId, tutorId) {
  const client = requireSupabase();
  if (!userId) throw new Error('Tu dois être connecté pour sauvegarder un tuteur.');
  if (!tutorId) throw new Error('tutorId requis.');

  const { data, error } = await client
    .from('favorite_tutors')
    .upsert(
      { user_id: userId, tutor_id: tutorId },
      { onConflict: 'user_id,tutor_id', ignoreDuplicates: true }
    )
    .select('id, user_id, tutor_id, created_at')
    .maybeSingle();

  if (error) {
    throw new Error(error.message || 'Impossible d’ajouter aux favoris.');
  }
...
/**
 * Retire un tuteur des favoris.
 */
export async function removeFavoriteTutor(userId, tutorId) {
  const client = requireSupabase();
  if (!userId) throw new Error('Tu dois être connecté.');
  if (!tutorId) throw new Error('tutorId requis.');
  ...
  if (error) {
    throw new Error(error.message || 'Impossible de retirer des favoris.');
  }
}

/**
 * Toggle favori. Retourne true si maintenant favori.
 */
export async function toggleFavoriteTutor(userId, tutorId, currentlyFavorite) {
  if (currentlyFavorite) {
    await removeFavoriteTutor(userId, tutorId);
    return false;
  }
  await addFavoriteTutor(userId, tutorId);
  return true;
}
